...
func _draw() -> void:
	...
	for enemy in enemies:
		...
		# Mostra solo i nemici sullo stesso livello di altezza del player!
		if "current_height_level" in enemy and "current_height_level" in player:
		...
		# Controlla se è all'interno della minimappa (con un piccolo margine di tolleranza)
		if enemy_pos.x > 3.0 and enemy_pos.x < size.x - 3.0 and enemy_pos.y > 3.0 and enemy_pos.y < size.y - 3.0:
	...
	for friend in friends:
		...
		# Mostra solo i nemici sullo stesso livello di altezza del player!
		if "current_height_level" in friend and "current_height_level" in player:
		...
		# Controlla se è all'interno della minimappa (con un piccolo margine di tolleranza)
		if friend_pos.x > 3.0 and friend_pos.x < size.x - 3.0 and friend_pos.y > 3.0 and friend_pos.y < size.y - 3.0:
	...
	for item in items:
		...
		# Mostra solo i nemici sullo stesso livello di altezza del player!
		if "current_height_level" in item and "current_height_level" in player:
		...
		# Controlla se è all'interno della minimappa (con un piccolo margine di tolleranza)
		if item_pos.x > 3.0 and item_pos.x < size.x - 3.0 and item_pos.y > 3.0 and item_pos.y < size.y - 3.0:
